...
from gym.common.protobuf.gym_grpc import AgentStub, MonitorStub, ManagerStub, PlayerStub, InfraStub
from gym.common.protobuf.gym_pb2 import Result, Task, Report, Instruction, Snapshot, Info, Deploy, Built

from gym.common.yang.vnfbd import VNFBD
from gym.common.yang.vnfpp import VNFPP
from gym.common.yang.vnfbr import VNFBR
...
class Core:
    ...
    async def _info(self, stub, contacts=[]):
        ...
        try:
            ...
            info_reply = json_format.MessageToDict(reply, preserving_proto_field_name=True)
                ...
                if role and address:
                    if role in roles:
                        ...
                        if host and port:
                            ...
                            logger.info(f"Host and/or port not provided -\
                                address must be ip:port (e.g., 127.0.0.1:8888)")
                    else:
                        logger.info(f"Unkown contact role -\
                            it must be one of {roles}")
                else:
                    logger.info(f"Role and/or address not provided -\
...
class WorkerCore(Core):
    ...
    async def instruction(self, message):
        ...
        instruction = json_format.MessageToDict(message, preserving_proto_field_name=True)
...
class ManagerCore(Core):
    ...
    def check_uuids(self, locals, requested):
        ...
        for req_id in req_ids:
            if req_id not in local_ids:
                logger.debug(f"Component ID requested {req_id} not existent {local_ids}")
                return False
    ...
    async def callInstructions(self, instructions, peers):
        ...
        for snap in snaps:
            if isinstance(snap, Exception):
                ...
                logger.info(f'Snapshot missing from peer {uuid} in instruction {instruction}')
    ...
    async def task(self, task):
        ...
        logger.info(f"Message: {json_format.MessageToDict(task, preserving_proto_field_name=True)}")
        ...
        instructions.update(self.instructions(monitors_peers, task.monitors, "monitors"))
            ...
            for vnfbd_instance in vnfbd.instances():
                ...
                if ack:                    
                    ...
                    for uuid,template in task_templates.items():
                        ...
                        logger.info(f"Task {task_ids} for test {test} with {trials} trials")
                        json_format.ParseDict(template, task)         
                        
                        report = await self.callTask(uuid, task)
                        report_dict = json_format.MessageToDict(report, preserving_proto_field_name=True)
