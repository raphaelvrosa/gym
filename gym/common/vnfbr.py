...
class VNFBR:
    ...
    def _extract_metrics(self, metrics, prefix, source, save=False):
        ...
        if save:
            dirname = "./csv"
            filename = "_".join(
                [prefix, source.get("type"), source.get("name"), ".csv"]
            )
    ...
    def _extract_evals(self, evals, prefix, test, trial, snap_role, save=False):
        ...
        for ev in evals:
            ...
            if type(ev_metrics) is list:
                ...
                metrics_series = [
                    metric for metric in ev_metrics if metric.get("series")
                ]
                if metrics_series:
                    metrics_series_summary = self._extract_metrics(
                        metrics_series, prefix, ev_metrics_source, save=save
                    )
        ...
        metrics_frmt = {
            "metric_" + snap_role + "_" + k: v for k, v in general_metrics.items()
        }
    ...
    def _extract_snaps(self, snaps, report_id, test, save=False):
        ...
        for snap in snaps:
            ...
            prefix = "_".join(
                ["report", str(report_id), "test", str(test), "trial", str(snap_trial)]
            )
            evals_metrics = self._extract_evals(
                evals, prefix, test, snap_trial, snap_role, save=save
            )
