    ...
    def collect_intf_statistics(self):
        ...
        for intf_name in intf_list:
            stat_files = os.listdir(
                os.path.join(self.NET_PATH, intf_name, "statistics")
            )
            for fn in stat_files:
                with open(
                    os.path.join(self.NET_PATH, intf_name, "statistics", fn), "r"
                ) as f:
                    data = -1
                    try:
                        data = float(f.read())
...
class ListenerSuricata(Listener):
    ...
    def process_diffs(self, stats_diffs, duration):
        ...
        stats_diffs_extra = {
            ...
            "query_avglen": (stats_diffs["query_size"] / stats_diffs["query_pkts"])
            - pkt_overhead,
            "reply_avglen": (stats_diffs["reply_size"] / stats_diffs["reply_pkts"])
            - pkt_overhead,
        }
    ...
    def stats_diff(self, interface, duration):
        ...
        stats_diffs = list(
            map(lambda x, y: x - y, stats_after.values(), stats_before.values())
        )
